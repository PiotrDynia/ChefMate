...
interface RecipeDetailsDao {
    ...
    @Query("DELETE FROM ingredient WHERE recipeId = :recipeId AND isInShoppingList = 0")
    suspend fun deleteIngredientsNotInShoppingList(recipeId: Int)

    @Query("SELECT * FROM ingredient WHERE id = :ingredientId AND isInShoppingList = 1 LIMIT 1")
    suspend fun getIngredientFromShoppingListById(ingredientId: Int) : IngredientEntity?
